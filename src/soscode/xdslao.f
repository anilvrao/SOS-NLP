      subroutine xdslao( mincor, diag  , xladj , ladjl , lnza  ,
     ...
     5                   stknod, rstbeg, istack, panel , lpanel, sqfil1,
     6                   sqtrn1, sqfil3, sqtrn3, wafil5, 
     7                   iopos2, wafil2, watrn2, locbfr, ocbufr, mxvlib,
     8                   mxvlrb, fctops, ierr )
c
...
c
c             this version is for symmetric problems.
c
c  created   -- 18-mar-97, rgg, derived from xdslai
c  revisions -- undated         added missing error check on return
c                               from xdsla7 or xdsla8
c               14-apr-03, jgl  tracks status of child processing
...
c      wafil5 -- i/o file to hold assemble front
c
c  working storge  --
c
c      panel  -- space to hold current section of front
...
c     -------------------
     ...
     3                  nstksv, phase , skip  , son   , iopsv2
      ...
      integer           actsav, lfront, loclfr, lpano , offset, 
     1                  pospon, poffst
...
c     ---------------------------------------------------------
      ...
      enddo
c.debug
c     write(6,'("in xdslao - iopsv2 = ", i8)') iopsv2
c.debug
...
c         colbgn to colend
c     ... start by zeroing out the panel
c     ------------------------------------------------------
...
c.debug
      ...
      if ( phase .eq. 1 ) then 
          ...
          call xdsla5 ( mincor, nodbgn, nodnxt, diag  , xladj,  
     1                  ladjl , lnza  ,
     2                  bassmb, sigma , bmxtyp, bdiag , bxladj,
     3                  bladjl, lnzb  , colbgn, i     , poffst,
     4                  lpano , loclfr, panel , sqfil1, sqtrn1, 
     5                  sqfil3, sqtrn3, qfirst, mxvlib, mxvlrb, 
     6                  fctops, ierr )
...
c         -------------------------------------------------------
          ...
          call xdsla7 ( nn1   , n2    , n3    , loclfr, lpan  , 
...
c         -------------------------------------------------
          ...
          call xdsla8 ( n1    , n2    , n3    , loclfr, lpan  , 
     1                  colbgn, colend, panel ,
     2                  nstack, nassmb, stknod, istack, lindxl,
...
c     ---------------------------------------
c.debug
c     write(6,'("before writing out the panel - iopos5, lpan = "
c    1     2i8)') iopos5, lpan
c     call xdslp5 ( 'panel', lpan, panel, 6 )
c.debug

      call xdslw2 ( wafil5, 2, idummy, idummy, panel, 
     1              iopos5, lpan, ierr )

      SUBROUTINE HSTMDN( K, NX, NEQNS, NDIM, KMAX, MINEQL, IPC, IPU,
     $                   GOTRHO, RHO, CHI, X0, IFREE0, BL, BU, INSHUR, 
     $                   ITPSHR, F0, GRAD0, W, AKIVF0, SOLVTL, NWKKT, 
     $                   WORKKT, NFRSLK, IER )
C
C ======================================================================
C     HSTMDN===>hstmdn   J.T. BETTS
C ======================================================================
C
...
C
          F0(NVFRE0-NFRSLK) = GRAD0( I - (MINEQL+1) )
C
...
C
          IF ( .NOT. GOTRHO ) THEN
            IER = 1
            IF ( IPC .GT. 0 ) THEN
              WRITE(IPU,*) ' HSTMDN:ERROR, GOTRHO=F BUT FEAS VAR IN KT'
            ENDIF
          ENDIF
C
          F0(NVFRE0-NFRSLK) = RHO + CHI*X0(MINEQL+1)
C
        ENDIF
C
      enddo
...
csolvtl        IF ( JER .NE. -701 .OR. DELTA .GT. SOLVTL ) THEN
        IF ( JER .NE. -701 ) THEN
          IER = 2
          IF ( IPC .GT. 0 ) THEN
            WRITE(IPU,*) ' HSTMDN: ERROR, JER IN HDSLSL=', JER
          ENDIF
        ENDIF
      ENDIF
C
...
C         "IF TEST" NEEDED BECAUSE SLACK VARS FIXED BY UPDATES
C         ARE NOT EXPLICITLY SOLVED FOR IN THIS VERSION OF THE CODE.
C         
          IF ( JVAR .GT. MINEQL ) THEN
            W(KK) = X0(JVAR) - BL(JVAR)
          ELSE
C           SLACK VAR
            W(KK) = BL(JVAR) - X0(JVAR)
          ENDIF
C
        ELSEIF( ITPSHR(KK) .EQ. -2 ) THEN
C
C         A VARIABLE SET TO ITS UPPER BOUND AT UPDATE KK.
C         "IF TEST" NEEDED BECAUSE SLACK VARS FIXED BY UPDATES
C         ARE NOT EXPLICITLY SOLVED FOR IN THIS VSERSION OF THE CODE.
C
          IF ( JVAR .GT. MINEQL ) THEN
            W(KK) = X0(JVAR) - BU(JVAR)
          ELSE
C           SLACK VAR
            W(KK) = BU(JVAR) - X0(JVAR)
          ENDIF
C
        ENDIF

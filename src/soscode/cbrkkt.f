      SUBROUTINE CBRKKT( NVAR, NEQNS, NRES, MSUBE, MSUBB, NSLK, IPU,
...
C                   SOLVER IS APPLIED TO A CONDENSED SYSTEM CONSTRUCTED FROM
C                   THE ORIGINAL UNSYMMETRIC KKT SYSTEM.
C
...
C
C                                                     
C |W + (B^T)(D_b)^(-1)D_l B   R^T     C^T  | | Dely  |     | Del L |
C |                                        | |       |     |       |
C |        R                   -I       0  | | Delz  | = - |   0   |
C |                                        | |       |     |       |
C |        C                    0       0  | |-Deleta|     |   c   |
C
C                   IN THE ABOVE:
C                      Del L  = (NVAR VECTOR) GRADIENT OF THE LAGRANGIAN
C                               AT PT Y, I.E. g(y)) -C^T*eta -B^T*LAMBDA
C                      c      = (MSUBE VECTOR) EQUALITY CONSTR VECTOR
...
C                      eta    = (MSUBE VECTOR) OF MULTIPLIERS FOR THE
C                               EQUALITY CONSTRS
C                      LAMBDA = (MSUBB VECTOR) OF MULTIPLIERS FOR
C                                BOUNDS CONSTRS
C                      D_l    = (MSUBB X MSUBB) DIAGONAL MATRIX OF LAMBDAS
C                      Dely   = (NVAR VECTOR) DELTA Y
C                      Delz   = (NRES VECTOR) DELTA Z, WHERE Z IS A VECTOR
C                               OF EXTRA VARIABLES, INTRODUCED TO PUT A
C                               LEAST-SQUARES PROBLEM INTO "AUGMENTED" FORM
C                      Deleta = (MSUBE VECTOR) DELTA eta
C                      DLMBDA = (MSUBB VECTOR) DELTA LAMBDA
...
C                                   BOUNDS CONSTRS
C                      D_b    = DIAGONAL MATRIX OF BOUNDS CONSTRS, I.E.
C                               D_b(I,I) = BVEC(I), FOR I=1,...,MSUBB.
C
C
C         INPUT:    
C
...
C                    I.E. G(Y) -C^T*eta -B^T*lambda (NVAR
C                    VECTOR) 
...
C             SOLNKT SOLUTION OF THE KKT SYSTEM (NEQNS+MSUBB)
C
...
C             -280 > IERBKT > -310       I/O ERRORS (INSUFFICIENT DISK
C                                                    SPACE)
C             -310 > IERBKT > -800       BAD CONDITIONING, WRONG
...
C                                        CALLING CHAIN
C
C             IERBKT IN CHRONOLOGICAL ORDER IN CBRKKT:
C
...
C                    -809  INCORRECT PATH (XDSLIC, IER=-120)
...
C                    -829  INCORRECT PATH (XDSLFA, IER=-190)             
...
C                    -736  EXCESSIVE FILL DUE TO PIVOTING (XDSLFA, IER=-509)                            
C                    -737  WRONG INERTIA OF KT MATRIX, AFTER CALL TO
C                          XDSLFA
C                    -838  INCORRECT PATH (XDSLSL, IER=-280)
...
C
      LOGICAL USEW
      DOUBLE PRECISION ADDLEV, WMATI, CDIAG
      ...
      DOUBLE PRECISION RTHBKT(NEQNS), SOLNKT(NEQNS+MSUBB)
...
C     ERRORS TO CBRKKT ERRORS (IERBKT).
C

      INTEGER KTERV(MXERRS), MFERV(MXERRS)
      DATA (MFERV(I), I=1, 39) / -120, -101, -102, -103, -104, -105,
...
C
      ELSEIF(MSUBE.LT.0) THEN
        IERBKT = -802
C
      ELSEIF(MSUBB.LT.0) THEN
        IERBKT = -803
C
      ELSEIF(NRES.LT.0) THEN
        IERBKT = -806
C
      ELSEIF(NVAR.LE.0) THEN
        IERBKT = -804
      ENDIF
      IF (IERBKT.LT.0)  GO TO 350
C
C
C     ESTIMATE FOR NUMBER OF NONZEROS IN LOWER TRIANGULAR PART
C     OF KKT MATRIX
C
      NZLTA = NONZR + NONZW + NONZC + NRES + MSUBE
C       
C     ------------------------------------------------------------
...
C       ...LOAD WMAT BLOCK
C
        jcwloop: DO I=JCOLW(JCOL), JCOLW(JCOL+1) - 1
          IF(NZCOL.NE.0..AND.WMAT(I).EQ.ZERO) cycle jcwloop
          NZCOL = NZCOL + 1
          ITEMP(NZCOL) = IROWW(I)
C
        enddo jcwloop
...
C
        IF(NRES.GT.0) THEN
          jcrloop: DO I=JCOLR(JCOL), JCOLR(JCOL+1) - 1
            IF(RMAT(I).EQ.ZERO) cycle jcrloop
            NZCOL = NZCOL + 1
...
C
        IF(MSUBE.GT.0) THEN
          jccloop: DO I=JCOLC(JCOL), JCOLC(JCOL+1) - 1
...
C
        ...
        ELSE
          ADDLEV = ADDHSS
        ENDIF
        jcolwloop: DO I=JCOLW(JCOL), JCOLW(JCOL+1) - 1
          IF(NZCOL.NE.0..AND.WMAT(I).EQ.ZERO) cycle jcolwloop
          NZCOL = NZCOL + 1
          ITEMP(NZCOL) = IROWW(I)
C
          IF(USEW) THEN
...
C
              IERBKT = -850
C
              IF(IPC.GT.0) WRITE(IPU,1003) JCOL
              GO TO 350
            ENDIF
C
C             COMPUTE MODIFICATION TO THE DIAGONAL, I.E. THE DIAGONAL 
C             MATRIX
C
C                   C = (B^T)(D_b)^(-1)(D_lambda)(B)
C
            CDIAG = ZERO
            IF(MSUBB.GT.0) THEN
              DO II = JCOLB(JCOL),JCOLB(JCOL+1)-1
                IROW = IROWB(II)
C
C               NOTE:  BMAT(II)**2 = 1, SO THE STATEMENT
C               CDIAG = CDIAG + (LAMBDA(IROW)*BMAT(II)**2)/BVEC(IROW)
C               CAN BE SIMPLIFIED TO
C
                CDIAG = CDIAG + LAMBDA(IROW)/BVEC(IROW)
              enddo
            ENDIF
C
...
C
            RTHBKT(NZCOL) = WMATI + ADDLEV + CDIAG
          ELSE
C
...
C
        IF(NRES.GT.0) THEN
          jcolrloop: DO I=JCOLR(JCOL), JCOLR(JCOL+1) - 1
            IF(RMAT(I).EQ.ZERO) cycle jcolrloop
            NZCOL = NZCOL + 1
            ITEMP(NZCOL) = NVAR + IROWR(I)
            RTHBKT(NZCOL) = RMAT(I)
          enddo jcolrloop
...
C
        IF(MSUBE.GT.0) THEN
          jcolcloop: DO I=JCOLC(JCOL), JCOLC(JCOL+1) - 1
            IF(CMAT(I).EQ.ZERO) cycle jcolcloop
            NZCOL = NZCOL + 1
            ITEMP(NZCOL) = NVAR + NRES + IROWC(I)
            RTHBKT(NZCOL) = CMAT(I)
          enddo jcolcloop
...
C
      IF(IERBKT.NE.0) GO TO 350
C
C     ...CHECK FOR ILL-CONDITIONED KKT MATRIX
C
...
C
      IF( INRTIA(1) .NE. (NEQNS-(NRES+MSUBE)) 
     $    .OR. INRTIA(2) .NE. (NRES+MSUBE)
...
C                                                  
C       GRADLB IS GRADIENT OF THE BARRIER LAGRANGIAN 
C         = g - (C^T) eta - (B^T) pib
C         = g - (C^T) eta - (B^T) lambda + (B^T) lambda - (B^T) pib
C         = gradl + (B^T) (lambda - pib)
C
      IF(MSUBB.GT.0) THEN
        DO I=1, MSUBB
C
...
C
          PIB = MU/BVEC(I)
          SOLNKT(I) = LAMBDA(I) - PIB
        enddo
C
C         COMPUTE (B^T) (lambda - pib) AND STORE IN RTHBKT
C
        CALL MVPSPR(11,NVAR,MSUBB,BMAT,IROWB,JCOLB,SOLNKT,RTHBKT)
C
      ELSE
        RTHBKT(1:NVAR) = ZERO
      ENDIF
C
      DO I=1, NVAR
C                                                  
C       GRADL IS GRADIENT OF THE LAGRANGIAN 
C         = g - (C^T) eta - (B^T) lambda
C
        RTHBKT(I) = -GRADL(I) - RTHBKT(I)
      enddo
C
C
...
C        (SIGN CHANGES MUST BE ACCOUNTED FOR.)
C
C
      DO I=1, NVAR
        JVAR = I
...
C
      IF(MSUBB.GT.0) THEN
C
C         COMPUTE THE VECTOR B(del y) IN DLMBDA
C
        CALL MVPSPR(1,MSUBB,NVAR,BMAT,IROWB,JCOLB,DY,DLMBDA)
C
C         COMPLETE CONSTRUCTION OF THE SOLUTION BY COMPUTING 
C
C         dlmbda = -(D_b)^(-1)(D_lamda)B(dely) - lambda + pib
C
        DO I=1, MSUBB
          DLMBDA(I) = - LAMBDA(I)*DLMBDA(I)/BVEC(I)
     $                - LAMBDA(I) + MU/BVEC(I)
        enddo
C
      ENDIF
...
C
C
      IF(IERBKT.NE.0.AND.IPC.GT.0) WRITE(IPU,1002) IERBKT
C
C     END KKT FACTORIZATION TIMING
C
      CALL CLKSUM(6)
C      
 1001 FORMAT(5X,'CBRKKT STORAGE ERROR;  NEEDS =',I6,'  LNRWRK =',I6)    
 1002 FORMAT(5X,'MULTIFRONTAL ERROR;  IERBKT =',I6)
 1003 FORMAT(5X,'ERROR: HESSIAN MISSING DIAGONAL IN COL NO. = ', I6)
C
      RETURN 
      END

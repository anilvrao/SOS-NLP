      subroutine xdslv1 ( matrix, irow,   jcol,   value,
     1                    work,   lwork,  error )
c
c     purpose
c     -------
c
c     xdslv1 is the top level driver for numeric input a single
c     element at a time.
c
...
c     ------------------------------
     ...
     4                    colstr, imaxnz, imxrec, ineqns, inuse ,
     5                    invp  , k     , kmatrx, lbound, maxnz , 
     6                    maxrec, msglvl, neqns , output, 
...
c     --------------------
      ...
      external            lsame , xdslv2, xdslni, xdslt1
...
c.debug
c         write(6,'("arcpos, arclen, brcpos, brclen, inuse = ",
c    1       5i8)')   arcpos, arclen, brcpos, brclen, inuse 
c         write(6,'("kmatrx                                = ",
c    1       5i8)')   kmatrx                                
...
c     -------------------------
      ...
      if ( jcol .lt.      1 .or. jcol .gt. neqns .or.
     1     irow .lt. lbound .or. irow .gt. neqns ) go to 8200
...
c     ----------------------
     ...
     1     lsame ( matrix(1:1), 'K' )    ) then
          ...
          call xdslv2 ( 1, lirow, ljcol, lvalu, unsym, neqns, anzero,
     1                  maxnz, anrecd, maxrec, wafile, work(invp), 
     ...
     1          lsame ( matrix(1:1), 'M' )      ) then
          ...
          call xdslv2 ( 1, lirow, ljcol, lvalu, unsym, neqns, bnzero,
     1                  maxnz, bnrecd, maxrec, wafile, work(invp), 
...
c     ---------------------------------------
 ...
 8100 continue
      error = -401
      call hherr ( 2, 'xdslv1', error, wkreqd )
...
88850 format ( /5x, '*** fatal error no. ', i5, ' *** invalid ',
     ...
     2         /5x, 'subroutine xdslv1.'
     3        /10x, 'matrix = ', a )
 
88900 format ( /5x, '*** fatal error no. ', i5, ' *** subroutine ',
     1              'xdslv1 encountered i/o error'

      subroutine xdslb5( mincor, nodbgn, nodnxt, 
     1                   diag  , xladj , ladjl , lnza  ,
     1                   bassmb, sigma , bmxtyp, bdiag , bxladj, 
     2                   bladjl, lnzb  , colbgn, colend, poffst, 
     3                   lpan  , loclfr, matsiz, panell, panelu, 
     4                   sqfil1, sqtrn1, sqfil3, sqtrn3, qfirst, 
     5                   mxvlib, mxvlrb, fctops, ierr )
c
...
c
c  created   -- 22-may-98, rgg -- derived from xdsla5
c
c  variables
c
...
c      matsiz -- size of all of the lower tri. front.  offset 
c                between lower tri. and upper tri. fronts.
...
c
c      panell -- array holding numerical values for section of 
c                current front - lower triangle
c      panelu -- array holding numerical values for section of 
c                current front - upper triangle
...
c
c      xdslb6 -- scatter/add a vector into another
c
c  =====================================================================
...
c.debug
c     write(6,'("in xdslb5 - starta, asize, mincor = ", 3i8)')
...
c.debug
                  ...
                  sqtrn1 = sqtrn1 + xdslni ( asize )
c.debug
c     write(6,'("after integer read in xdslb5")')
...
c.debug
c     write(6,'("before out-of-core call to xdslb6")')
c     write(6,'("poffst, lpan = ", 2i8)') poffst, lpan
c     call xdslp5 ( 'org. entries', asize, mxvlrb, 6 )
c     call xislp3 ( 'org. entries', asize, mxvlib, 6 )
c.debug
              call xdslb6 ( one, asize, mxvlrb, mxvlib, 
     1                      poffst, lpan, matsiz, panell, panelu ) 
c.debug
c     write(6,'("after call to xdslb6")')
c     call xdslp5 ( 'panel', lpan, panell, 6 )
c     call xdslp5 ( 'panel', lpan, panelu, 6 )
c.debug
...
c.debug
          panell(k) = panell(k) + diag (node+nodoff)
          k         = k + length
          length    = length - 1
      enddo
...
c             -----------------------------------------
              ...
              do node = colbgn, colend 
                  panell(k) = panell(k) - sigma * bdiag(node+nodoff)
                  k         = k + length
                  length    = length - 1
              enddo
...
c.debug
                      ...
                      call xdslb6 ( -sigma, bsize,
     1                               mxvlrb(asize+1), mxvlib(asize+1), 
     2                               poffst, lpan, matsiz, 
     3                               panell, panelu ) 
c.debug
c     write(6,'("after call to xdslb6")')
c     call xdslp5 ( 'panell', lpan, panell, 6 )
...
c.debug
                  ...
                  call xdslb6 ( -sigma, bsize, lnzb(startb),
     1                          bladjl(startb), poffst, lpan, 
     2                          matsiz, panell, panelu ) 
c.debug
c     write(6,'("after call to xdslb6")')
c     call xdslp5 ( 'panell', lpan, panell, 6 )
c     call xdslp5 ( 'panelu', lpan, panelu, 6 )

      ...
      SUBROUTINE BLINES(QZERO,QPRIME,QBAR,ALFA,ALFBND,PENMU,CBTCB,
     $    C0TC0,CNORM,IT,MAXIT,NWFLTR,IFUNER,IOFLAG,
     $    IPU,IFC,IERLIN,SAMSCL)
C
C
C ======================================================================
C     BLINES===>blines   J.T. BETTS
C ======================================================================
C
...
C                           POINT
C                   = 6     FEASIBLE POINT (FEASIBILITY MODE)
C                   = 7     MAX. NUMBER OF F.E.
C                   = 8     FILTER REJECTS POINT WITH UNEQUAL PRIMAL-DUAL SCALING
C                   ----FATAL ERRORS
C                   = -1    QPRIME > 0
C                   = -2    ALFBND .LE. 0
...
C
      ...
      CALL HHCPRS(TYTLE,' ','.')
      IF(IOFLAG.GE.10) WRITE(IPU,1003) TYTLE(1:32),IT,ALFA
      IFC = 1
      IFESR = 1
C
...
C
      IF(IFUNER.NE.1) THEN
        ...
        ELSEIF(NWFLTR.GT.0) THEN
          FLTOUT(28:31) = '<  >'
          WRITE(FLTOUT(29:30),'(I2)') NWFLTR
        ENDIF
        IF(IOFLAG.GE.10) WRITE(IPU,1002) FLTOUT(1:44),QBAR,CNORM
      ENDIF
C
C         FEASIBLE POINT (FEASIBILITY MODE)
C
      IF(IFUNER.EQ.-101) THEN
        IERLIN = 6
        GO TO 130
      ENDIF
C
...
C
 1001 FORMAT(T3,'*',T106,'*'/T3,'*',T11,'.........................Functi
     $on Error',T106,'*')
 1002 FORMAT(T3,'*',T106,'*'/T3,'*',T11,A44,T55,SP,1PE14.6,',',
     $   SP,1PE14.6,' )',T106,'*')
 1003 FORMAT(T3,'*',T106,'*'/T3,'*',T11,A32,'Step:',I3,'.....ALFA =',
     $   1PG16.8,T106,'*')
      RETURN
      END

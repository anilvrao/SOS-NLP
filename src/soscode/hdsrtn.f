      SUBROUTINE HDSRTN ( A, N, IOPT, ISTBLE, INDX, IER )
C
C----------------------------------------------------------------------
C
C  PURPOSE  HDSRTN SORTS A DOUBLE PRECISION ARRAY OF N ELEMENTS INTO
C           THE ASCENDING ORDER AND GENERATES THE PERMUTATION
C           ARRAY INDX.
C
C  METHOD   HDSRTN USES A QUICKSORT ALGORITHM IN THE STYLE OF THE
...
C     PARTITIONING.
C
C ... SET INDX TO THE IDENTITY PERMUTATION
C
      DO I = 1, N
         INDX(I) = I
      ENDDO
...
C
 4000 CONTINUE
      IF ( IOPT .NE. 0 ) THEN
C
        CALL HDPRMY ( A, N, INDX, IER )
        IF( IER .NE. 0 ) THEN
C
C ... UNEXPECTED ERROR RETURN FROM HDPRMY
C
          IER = -7
          GO TO 8300
        ENDIF

      subroutine xdslea( zpcntl, npcntl, pnlcol, pnlrow, panel,
...
c     --------------------
      ...
      double precision  zpcntl(*), npcntl(*), panel(pnlcol,pnlrow)
...
c  -------------------------------------------
     ...
     1                  pnlrow-m+2, panel(m-2,m-1), pnlcol,
...
c  -------------------------------------------
          fac21          = panel(m-2,m-1)
          panel(m-2,m-1) = rdiag1*panel(m-2,m-1)
          panel(m-1,m-1) = panel(m-1,m-1) - fac21*panel(m-2,m-1)

          call xdslep ( m-1, panel(m-1,m-1), 
     1                  pnlrow-m+1, panel(m-1,m), pnlcol,
     2                  zpcntl, npcntl, inrtia, ierr )
          if ( ierr .ne. 0 )  then
             if  ( ( ierr .eq. -1 ) .or. ( ierr .eq. -2 ) )  then
                error = ierr
...
c  ------------------------------------------
          fac31        = panel(m-2,m)
          panel(m-2,m) = rdiag1*panel(m-2,m)
          fac32        = panel(m-1,m) - fac21*panel(m-2,m)
          panel(m-1,m) = rdiag2*fac32
          panel(m  ,m) = panel(m  ,m) - fac31*panel(m-2,m) 
     1                                - fac32*panel(m-1,m)

          call xdslep ( m, panel(m,m), 
     1                  pnlrow-m, panel(m,m+1), pnlcol,
     2                  zpcntl, npcntl, inrtia, ierr )
          if ( ierr .ne. 0 )  then
             if  ( ( ierr .eq. -1 ) .or. ( ierr .eq. -2 ) )  then
                error = ierr
...
cdir$ ivdep
          do i = m+1, pnlrow
              panel(m-2,i) = rdiag1* panel(m-2,i)
              panel(m-1,i) = rdiag2*(panel(m-1,i) - fac21*panel(m-2,i))
              panel(m  ,i) = rdiag3*(panel(m  ,i) - fac31*panel(m-2,i) 
     1                                            - fac32*panel(m-1,i))
          enddo
...
c         -------------------------------------
          ...
          do j = m+1, pnlcol-1, 2
              ...
              fac11 = diag1*panel(m-2,j  )
              fac12 = diag1*panel(m-2,j+1)
              fac21 = diag2*panel(m-1,j  )
              fac22 = diag2*panel(m-1,j+1)
              fac31 = diag3*panel(m  ,j  )
              fac32 = diag3*panel(m  ,j+1)

              panel(j,j) = panel(j,j) - fac11*panel(m-2,j) 
     1                                - fac21*panel(m-1,j) 
     2                                - fac31*panel(m  ,j)

cdir$ ivdep
              do i = j+1, pnlrow
                  panel(j  ,i) = panel(j  ,i) - fac11*panel(m-2,i)
     1                                        - fac21*panel(m-1,i) 
     2                                        - fac31*panel(m  ,i)
                  panel(j+1,i) = panel(j+1,i) - fac12*panel(m-2,i)
     1                                        - fac22*panel(m-1,i) 
     2                                        - fac32*panel(m  ,i)
              enddo

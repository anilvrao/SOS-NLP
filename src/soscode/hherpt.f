...
C     SUBROUTINE HHERR TO RESUME THE STATUS OF ERROR COUNTING AND
...
C                        PRINTING CAPABILITIES OF HHERR.
C
C      IF ITYP.EQ.1  THEN
C                     IF ERROR COUNTING IS TURNED OFF THEN
C                          REVERT TO STATUS PRIOR TO LAST HHERPT CALL.
...
C
      ...
      IF(ITYP.EQ.0) THEN
       MODE = -5
       ENDIF
      IF( (ITYP.GE.0) .AND. (ITYP.LE.1) ) CALL HHERR(MODE,A,B,C)
      RETURN
      END

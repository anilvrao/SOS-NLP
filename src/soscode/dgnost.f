      SUBROUTINE DGNOST( XBAR   ,XLWR   ,XUPR   ,NDIM   ,NRES   
     $          ,DELF   ,RMAT   ,IROWR  ,JCOLR  ,NONZR     
     $          ,IROWH  ,JSTRH  ,NONZH  ,MCON   ,MTOTAL ,IPRMC        
     $          ,GMAT   ,IROWG  ,JCOLG  ,NONZG  ,NONZGT ,WORK   
     $          ,NWORK  ,IWORK  ,NIWORK ,BIGELM ,IROWBG ,JCOLBG 
     $          ,LENBIG ,MINOPT ,IERNLP ,PRMUTE)
C
C ======================================================================
C     DGNOST===>dgnost   J.T. BETTS
C ======================================================================
C
...
C       MTOTAL      NUMBER OF CONSTRAINTS EXCLUDING IGNORED
...
C       NONZGT      NUMBER OF NONZEROS IN NONIGNORED JACOBIAN
C       WORK        WORK ARRAY 
...
C                   (NIWORK .GT. MAX(NONZR,NONZG,NDIM+1) )
...
C
     ...
     $      ONEEM1=1.0D-1,ONEEM2=1.0D-2,ONEEM4=1.0D-4)
C
...
C
      IABPAT = ABS(IOFPAT)
      IF((IOFLAG.GE.30.OR.IABPAT.GE.10).AND.(QPOPTN.EQ.'SPARSE'))  THEN
        IF(IABPAT.EQ.10) THEN
          IPUINT = -IPUNLP
        ELSE
          IF(IABPAT.EQ.IPUDRF.OR.IABPAT.EQ.IPUFZF.OR.IABPAT.EQ.IPUMF1
     $   .OR.IABPAT.EQ.IPUMF2.OR.IABPAT.EQ.IPUMF3.OR.IABPAT.EQ.IPUMF4
     $   .OR.IABPAT.EQ.IPUMF5.OR.IABPAT.EQ.IPUMF6.OR.IABPAT.EQ.IPUSTF)
     $    THEN 
            IPUINT = -IPUNLP 
          ELSE
            IPUINT = IABPAT
...
C
        IF(MTOTAL.GT.0) THEN
          TITLE = BLANK
      TITLE(25:74) = 'JACOBIAN MATRIX SPARSITY PATTERN (Internal Order)'
          CALL SPRPAT(1,IROWG,JCOLG,NDIM,MTOTAL,NONZGT,IPUINT,TITLE)
        ENDIF
C
        IF(NDIM.GT.0.AND.MINOPT.LT.4) THEN
          TITLE = BLANK
          IF(NRES.GT.0) THEN
...
C
        IF(NRES.GT.0) THEN
          ...
          CALL SPRPAT(1,IROWR,JCOLR,NDIM,NRES,NONZR,
     $              IPUINT,TITLE)
        ENDIF
C
      ENDIF
...
C
      WORK(1:NDIM) = ZERO
      IF(MTOTAL.GT.0) CALL MNMSPR(-2,MTOTAL,NDIM,GMAT,IROWG,
     $    JCOLG,WORK)
C
...
C
      IF(NRES.GT.0) THEN
C
        WORK(NDIM+1:NDIM+NDIM) = ZERO
        CALL MNMSPR(-2,NRES,NDIM,RMAT,IROWR,JCOLR,
     $               WORK(NDIM+1))
C
      ENDIF
C
...
C
        ...
        ENDIF
C
        IF(MINOPT.LT.4.OR.MINOPT.EQ.5) WORK(I) = 
     $                    MAX(WORK(I),OBJTRM)
C
...
C          
      IF(MTOTAL.GT.0) THEN
C
        CALL MNMSPR(-1,MTOTAL,NDIM,GMAT,IROWG,JCOLG,WORK)
C
C         LOAD NEGATIVE VALUE INTO IGNORED ROW NORMS
C
        IF(MCON.GT.MTOTAL) WORK(MTOTAL+1:MCON) = -ONE
C
C         PERMUTE ROW NORM VECTOR TO EXTERNAL ORDER
C
        IF(PRMUTE) THEN
          CALL HDPRMY(WORK,MCON,IPRMC,IERP)
          IF(IERP.NE.0) THEN
            IERNLP = -147
            GO TO 210
          ENDIF
...
C
      IF(IOFLAG.GE.10.AND.NLARGE.NE.0) THEN
        WRITE(IPUNLP,1006) 
        CALL INTOUT(IWORK(MTOTAL+1),NLARGE,IPUNLP)
      ENDIF
C
      NSMALR = 0
      NLARGR = 0
...
C
      IF(IOFLAG.GE.10.AND.NLARGR.NE.0) THEN
        ...
        CALL IRLOUT(IWORK(MTOTAL+1),WORK,NLARGR,IPUNLP)
        WRITE(IPUNLP,1016)
      ENDIF
C
      IF(MINOPT.LT.4.OR.MINOPT.EQ.5) THEN
C
        IF(NRES.GT.0) THEN
C
...
C         JACOBIAN
C          
          CALL MNMSPR(-1,NRES,NDIM,RMAT,IROWR,JCOLR,WORK)
C
          NSMALL = 0
          NLARGE = 0
C
...
C
 1005 FORMAT(T3,'*',T106,'*'/T3,'*',T11,'.....THE FOLLOWING ROWS OF THE  
     ...
     $   /T3,'*',T106,'*')
C
 1006 FORMAT(T3,'*',T106,'*'/T3,'*',T11,'.....THE FOLLOWING ROWS OF THE  

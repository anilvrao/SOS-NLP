      subroutine xdslu2( pvttol, mincor, fpnlsz, fupdsz, 
     1                   neqns , diag  , xladj , ladjl , lnza  ,
     1                   sigma , bmxtyp, bdiag , bxladj, bladjl,
     2                   lnzb  , extfil, zpcntl, npcntl, perm  , 
     2                   nsuper, xsup  , sup   , nassmb, npanel,
     3                   xpanel, xlindx, lindxl, lndxg1, nsninx, iwork ,
     4                   rwork , stkbgn, lstack, lnzbgn, llnz,   qincor,
     5                   mstack, stknod, rstbeg, istack, sqfil1, sqfil3,
     6                   wafil1, wafil2, wafil4, wafil5, 
     6                   lbuffr, ocbufr, mxvlrb, mxvlib,
     7                   fnzlf , nsnind, pvtblk, walen1, walen2, 
     8                   walen4, walen5, sqtrn1, sqtrn3, 
     8                   watrn1, watrn2, watrn4, watrn5, mpanel, 
     9                   fctops, slvops, error , ppfmon, output )
c
c  purpose -- to get a ldu factorization of a unsymmetric sparse matrix
...
c
c             if sigma .ne. 0. then xdslu2 factors a - sigma * b
c             where bmxtyp determines the matrix
c             type of b (0 - null, 1 - general, 4 - general but same
c             structure as a, 3 - diagonal, and 4 - identity).
c
c  created   -- 19-may-98, rgg, derived from xdslf2
c  revisions -- 01-sep-98, rgg, 32 bit integer mods
c               08-mar-00, rgg, allowed for possible increase of factor
c                               panel size for factorization of out of
c                               memory fronts (xdsldo) to reduce i/o.
...
c      lnza   -- entries of the original matrix by chevrons
c      sigma  -- shift value
...
c      lnzb   -- entries of the b matrix by chevrons
...
c      wafil4 -- word addressable i/o file for unz
...
c                the call to xdslu2.  iwork contains the new global
...
c      walen4 -- length of i/o file wafil4
...
c      watrn4 -- amount of i/o transfered to and from i/o file wafil4
...
c      xdslu3 -- spills stack of update matrices to wafil2.
c      xdslbi -- in-core frontal matrix assembly routine
c      xdslbo -- out-of-core frontal matrix assembly routine
c      xdsldi -- in-core elimination routine - unsymmetric
c      xdsldo -- out-of-core elimination routine - unsymmetric
...
c     --------------------
     ...
     2                  walen4, watrn4, walen5, watrn5, 
     3                  sqtrn1, sqtrn3, extfil, fnzlf
...
c     --------------------
      ...
      external          xdslil, xdslw2, xdslmv, 
     1                  xdslni, xdslu3, xdslbi, xdslbo, xdsldi
c
c  =====================================================================
...
c    1            updsiz                        
c     write(6,'("qincor, walen1, fnzlf,  stkoff = ", l8,3l8)')
c    1            qincor, walen1, fnzlf,  stkoff
c.debug
...
c                    -----------------------------------------------
                     ...
                     l      = ncolpp + lsuper*(lsuper+1)/2 + 2 * l
...
c         ---------------------------------------
          ...
          k = fnzlf + 2 * ( lfront*nnode - nnode*(nnode+1)/2 ) 
...
c             -----------------------------
              ...
              if ( wafil1 .lt. 0 .or. wafil4 .lt. 0 ) then
                  if ( wafil2 .ge. 0 ) go to 100
                  error = -7
                  return
              end if
 
c             ---------------------------------------------
c             ... write lnz out to wafil1 and unz to wafil4
c             ---------------------------------------------
              ...
              call xdslfb ( .true., isuper-1, npanel, xsup, xlindx,
     1                      xpanel, rwork(lnzbgn), wafil1, wafil4,
     2                      ierr )
c.debug
c     write(6,'("after xdslfb - ierr, fnzlf = ", 2i8)') ierr, fnzlf
c.debug
 
              if ( ierr .ne. 0 ) then
                  if ( ierr .eq. 1 ) error = -5
                  if ( ierr .eq. 2 ) error = -12
                  return
              endif
...
c.debug
              ...
              call xdslu3 ( kstack, stknod, rstbeg, xlindx,
...
c     write(6,'("after xdslu3 -       iopos2         = ",2i8)') 
c    1            iopos2            
c.debug
...
c     write(6,'("pnlsiz, fstore, lstack, llnz   = ", 4i8)')
c    1            pnlsiz, fstore, lstack, llnz  
c     write(6,'("updsiz, iopos1, iopos2, stneed = ", 4i8)')
c    1            updsiz, iopos1, iopos2, stneed
c.debug

          if ( stneed .le. lstack ) then
              qicasm = .true.
          else if ( wafil1 .ge. 0 .and. wafil4 .ge. 0 
     1                            .and. wafil2 .ge. 0 ) then
              qicasm = .false.
          else
              if ( wafil2 .lt. 0 ) error = -8
              if ( wafil1 .lt. 0 .or. wafil4 .lt. 0 ) error = -7
              return
          end if
c.debug
c         qicasm = .false.
...
c             --------------------
              ...
              panell = stfrnt + 2 * matsiz
              panelu = panell + pnlsiz * lfront
              temp1  = panelu + pnlsiz * lfront
              stkuse = temp1  + updsiz * lfront
c.debug
c     write(6,'("ica-panell, temp1, stfrnt, stkuse, lstack = ", 5i8)')
c    1                panell, temp1, stfrnt, stkuse, lstack 
c     write(6,'("ica-panelu, swap                          = ", 5i8)')
c    1                panelu, swap                          
c.debug
...
c.debug
              ...
              if ( nstack .eq. 0 .or. rstbeg(nstack) .eq. 0 ) then 
                  nwfree = stfree
              else
                  nwfree = rstbeg(nstack)
              end if
 
              call xdslbi (    mincor, diag  , xladj , ladjl , lnza  ,
...
c             ------------------------
              ...
              panell = stfree
              lpanel = ( lstack - panell + 1 ) / 2
              panelu = panell + lpanel
              stkuse = lstack
c.debug
c     write(6,'("oca-panell, panelu, lpanel, stkuse       = ", 5i8)')
c    1                panell, panelu, lpanel, stkuse
c.debug
...
c     write(6,'("pre -xdslbo - iopos2                 = ", 3i8)')
c    1                          iopos2                        
...
c.debug
              ...
              watrn5 = watrn5 + 2 * matsiz
              if ( 2 * matsiz .gt. walen5 ) walen5 = 2 * matsiz
...
c    1            pnlsiz, updsiz
c     call xdslp5('lower front before factor', lfront*(lfront+1)/2, 
c    1            rwork(stfrnt+stkoff), 6 )
c     call xdslp5('upper front before factor', lfront*(lfront+1)/2, 
c    1            rwork(stfrnt+matsiz+stkoff), 6 )
c     write(6,'("pre -xdsldi - isuper, fnzlf , iopos1 = ", 4i8)')
c    1                          isuper, fnzlf , iopos1        
c.debug
 
              call xdsldi ( pvttol, lfront, nnode, pnlsiz,
     1                      updsiz, pospon, nodoff, zpcntl, npcntl,
     2                      sup(bgxsup),    rwork (stfrnt+stkoff), 
     2                      rwork (stfrnt+stkoff+matsiz), 
     3                      pvtblk(bgxsup), rwork(panell+stkoff), 
     3                      rwork(panelu+stkoff), 
     4                      rwork(swap+stkoff), rwork(temp1+stkoff),    
     4                      fnzlf,  npanel, 
     5                      xpanel, mpanel, qincor, diag(bgxsup),   
     6                      rwork(lnzbgn+k),    iopos1, wafil1, 
     7                      wafil4, ocbufr, lbuffr, fctops, 
...
c.debug
c     write(6,'("post-xdsldi - isuper, fnzlf , iopos1 = ", 4i8)')
c    1                          isuper, fnzlf , iopos1        
c     write(6,'("post-xdsldi - hldnnd, nnode, ierr    = ", 3i8)')
c    1                          hldnnd, nnode, ierr
c     call xdslp5('lower front after factor', lfront*(lfront+1)/2, 
c    1            rwork(stfrnt+stkoff), 6 )
c     call xdslp5('upper front after factor', lfront*(lfront+1)/2, 
c    1            rwork(stfrnt+stkoff+matsiz), 6 )
c.debug
...
c             -------------------------------------------------------
     ...
     1                 - ( updsiz * lfront + xdslni ( lfront ) )
             ...
             if ( lavail .ge. 2 * pnlsiz * lfront ) then

c               ------------------------------------------
c               ... enlarge panel size as much as possible
c               ------------------------------------------
                
                pnlsiz = min ( nnode, lavail / (2*lfront) )
...
c             -------------------------
              ...
              panell = stfree 
              panelu = panell + pnlsiz * lfront
              swap   = panelu + pnlsiz * lfront
              temp1  = swap   + xdslni ( lfront )
              stkuse = temp1  + updsiz * lfront
c.debug
c     write(6,'("oce-panell, temp1 , stkuse, lfront, pnlsiz= ", 5i8)')
c    1                panell, temp1 , stkuse, lfront, pnlsiz
c     write(6,'("oce-stkoff, stkbgn, updsiz, panelu       = ", 5i8)')
c    1                stkoff, stkbgn, updsiz, panelu       
c.debug
...
c.debug
c     write(6,'("pre -xdsldo - isuper, fnzlf , iopos1 = ", 4i8)')
c    1                          isuper, fnzlf , iopos1        
c     write(6,'("pre -xdsldo - iopos2                 = ", 3i8)')
c    1                          iopos2                        
c     call xislp3 ( 'lindx2 pre xdsldo', bgdxg2-1, iwork, 6 )
c.debug
              iopsv2 = iopos2
              lextra = lstack - ( stkuse - 1 )
c.debug
c     write(6,'("pre -xdsldo - lbuffr, lextra         = ", 3i8)')
c    1                          lbuffr, lextra                
c.debug

              if ( lbuffr .ge. lextra ) then

              call xdsldo ( pvttol, lfront, nnode, pnlsiz, updsiz,
     1                      matsiz, pospon, nodoff, zpcntl, npcntl,
     2                      sup(bgxsup),            pvtblk(bgxsup),
     3                      rwork(panell+stkoff),   
     3                      rwork(panelu+stkoff),
     3                      rwork(swap+stkoff),
     4                      rwork(temp1+stkoff),    fnzlf,  npanel, 
     5                      xpanel, mpanel, diag(bgxsup),   
     5                      wafil5, watrn5, 
     6                      iopos1, wafil1, watrn1, wafil4, watrn4, 
     7                      iopos2, wafil2, walen2, watrn2, ocbufr, 
     8                      lbuffr, fctops, slvops,
...
c.debug
c     write(6,'("before call to xdsldo using extra")')
...
c.debug
     ...
     5                      wafil5, watrn5, 
     6                      iopos1, wafil1, watrn1, wafil4, watrn4, 
     7                      iopos2, wafil2, walen2, watrn2, 
     7                      rwork(stkuse+stkoff), 
     8                      lextra, fctops, slvops,
     8                      ppfmon, xpboxs, psboxs,
     9                      inpexp, inpsiz, inzsiz, izfail, rtpexp,
     A                      rtpsiz, rtzsiz, rzfail, ierr )
...
c.debug
c     write(6,'("post-xdsldo-isuper, fnzlf , iopos1, ierr = ", 5i8)')
c    1                        isuper, fnzlf , iopos1, ierr        
c     write(6,'("post-xdsldo-hldnnd, nnode                = ", 3i8)')
c    1                        hldnnd, nnode       
c     call xislp3 ( 'lindx2 post xdsldo', bgdxg2-1, iwork, 6 )
c.debug
              if ( qzpvt1 .and. zpcntl(3) .ne. 0 ) then
                  qzpvt1    = .false.
                  k         = zpcntl(3) + nodoff
...
c     write(6,'("before recording of postponed columns - ",
c    1          "ncolpp, nnode = ", 2i8)') ncolpp, nnode
c     end if
c.debug
...
c             ------------------------------------------
              ...
              length = matsiz - k
              startl = stfrnt + matsiz - length
              startu = startl + matsiz 
c.debug
c     write(6,'("stacking - k, matsiz, length, nstack = ",
c    1            5i8)')     k, matsiz, length, nstack 
c     write(6,'("stacking - startl, startu            = ",
c    1            5i8)')     startl, startu            
c.debug
...
c     if ( ncolpp .gt. 0 ) then
c     call xdslp5 ( 'lower matrix to stack', 
c    1              length, rwork(startl+stkoff), 6 )
c     call xdslp5 ( 'upper matrix to stack', 
c    1              length, rwork(startu+stkoff), 6 )
c     end if
c.debug
...
c.debug
c     if ( ncolpp .gt. 0 ) then
c     call xdslp5 ( 'lower matrix before stacking', length, 
c    1              rwork(startl+stkoff), 6 )
c     call xdslp5 ( 'upper matrix before stacking', length, 
c    1              rwork(startu+stkoff), 6 )
c     end if
c.debug
              call xdslmv ( length, rwork, startl+stkoff, 
     1                                     stfree+stkoff )
              call xdslmv ( length, rwork, startu+stkoff, 
     1                                     stfree+stkoff+length )
c.debug
c     if ( ncolpp .gt. 0 ) then
...
c     write(6,'("nstack                         = ", 4i8)')
c    1            nstack                         
c     call xdslp5 ( 'stack', stfree-1, rwork, 6 )
c.debug
...
68004 format (/10x, 'total time in xdslu2       ', 18x, f12.3 
